...
class TestCString(unittest.TestCase):
    ...
    def test_modify_value_long_string(self):
        """Testing re-assignment with a string that exceeds the size (should truncate)."""
        s   = Cstring("Nothing in here", 5)
        s   = "Python and a lot more text"
        self.assertEqual(s, "Pytho")
        self.assertEqual(s.size, 5)
        
        s.data = "New Data"
        self.assertEqual(s, "New D")
        self.assertEqual(s.size, 5)
    ...
    def test_repr_representation(self):
        ...
        s = Cstring("Hello", 5)
        self.assertEqual(repr(s), "Cstring(data='Hello', size=5)")
