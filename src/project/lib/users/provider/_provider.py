...
#   "find_provider_by_attributes"
#
def find_provider_by_attributes(providers, **attributes):
    return next(
        (provider for provider in providers
         if all(getattr(provider, attr) == value for attr, value in attributes.items())),
        None
    )
...
###############################################################################
    ...
    def from_dict(cls, data):
        provider            = cls( name=data['name'],           id=data['id'],
                                   address=data['address'],     city=data['city'],
                                   state=data['state'],         zip=data['zip'] )
        provider.history    = [Service.from_dict(record) for record in data.get("history", [])]
        return provider
